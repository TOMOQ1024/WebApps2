...
export default class GLMgrBase {
  ...
  lightPosition = new Vector3(4.0, 0.0, 0.0);
  lightDirection = new Vector3(-0.3, 0.5, 0.7).normalize();
  ambientColor = [0.1, 0.1, 0.1, 1.0];

  constructor(options?: { createShaders: Function }) {
    if (options) {
  ...
  setUniform2FV(name: string, v: Vector2) {
    this.gl!.uniform2fv(this.uniLoc[name], v.toArray());
  }

  setUniform3FV(name: string, v: Vector3) {
    this.gl!.uniform3fv(this.uniLoc[name], v.toArray());
  }
}
