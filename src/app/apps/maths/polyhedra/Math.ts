...
export class GyrovectorSpace2 {
  ...
  mul(r: number, P: Vector2) {
    if (r === 0 || P.length() === 0) return new Vector2(0, 0);
    return P.clone()
      .normalize()
  ...
  rotateFrom(P: Vector2, O: Vector2, a: number) {
    return this.add(O, this.rotate(this.sub(P, O), a));
  }
  ...
  angle2(P: Vector2, Q: Vector2) {
    let s = Math.sign(P.x * Q.y - P.y * Q.x);
    if (Math.abs(s) < 1e-6) s = 1;
    return s * Math.acos(this.dot(P, Q) / P.length() / Q.length());
  }

  angle3(P: Vector2, Q: Vector2, R: Vector2) {
    return this.angle2(this.sub(P, Q), this.sub(R, Q));
  }
  ...
  line(V: Vector2, P: Vector2, Q: Vector2) {
  ...
  segment(V: Vector2, P: Vector2, Q: Vector2) {
    if (P.distanceToSquared(Q) < 1e-6) return 9999 * V.distanceTo(P);
  ...
  inradius(P: Vector2, Q: Vector2, R: Vector2) {
  ...
  incenter(P: Vector2, Q: Vector2, R: Vector2) {
    const QR = this.distance(Q, R);
    const RP = this.distance(R, P);
    const PQ = this.distance(P, Q);
    const RD = this.atan(
  ...
  area(P: Vector2, Q: Vector2, R: Vector2) {
    if (this.curvature === 0) {
  ...
  v_oss(P: Vector2, Q: Vector2, R: Vector2) {
