...
export default function LinearAttractor2DApp() {
  ...
  // 点群を原点に初期化する関数
  const initializeToOrigin = useCallback(() => {
  ...
  // ランダムサンプル適用
  const applyRandomSample = useCallback(() => {
    const keys = Object.keys(sampleParams) as (keyof typeof sampleParams)[];
  ...
  return (
    <main className="w-screen h-[calc(100vh-var(--header-height))]">
      <ControlButtons
        ...
        onRandomSample={applyRandomSample}
      />
      {controlsVisible && (
        <div className="absolute bottom-[10px] left-[10px] z-10">
          ...
          <div className="bg-[var(--background-color)] p-[10px] border-2 flex flex-row gap-4 mb-[10px]">
            {([0, 1, 2, 3] as const).map((i) => (
              <div key={`transform-${i}`} className="flex flex-col items-start">
                <RealNumberInput
                  value={
