...
export type ExpressionMode = "explicit" | "implicit";
...
export class Graph3DCore {
  ...
  private surfaceMesh: Mesh<BufferGeometry, MeshStandardMaterial> | null = null;
  private material: MeshStandardMaterial;
  private gridHelper: GridHelper;
  private axesHelper: AxesHelper;
  private initialCameraPosition: Vector3;
  private initialCameraTarget: Vector3;
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    // Setup camera - positioned to see Z as up
    // Camera looks from a diagonal angle
    ...
    // Position camera so Z appears as vertical axis
    this.camera.position.set(8, 6, 8);
    this.camera.up.set(0, 0, 1); // Set Z as up direction
    this.camera.lookAt(0, 0, 0);
    this.initialCameraPosition = this.camera.position.clone();
    this.initialCameraTarget = new Vector3(0, 0, 0);
    ...
    directionalLight.position.set(5, 5, 10); // Light from above (Z direction)
    directionalLight.castShadow = true;
    this.scene.add(directionalLight);

    // Setup grid helper - rotate to XY plane (Z is up)
    this.gridHelper = new GridHelper(20, 20);
    this.gridHelper.rotation.x = Math.PI / 2; // Rotate to XY plane
    this.scene.add(this.gridHelper);

    // Setup axes helper (X=red, Y=green, Z=blue)
  ...
  /**
   * Update the surface mesh for explicit function z = f(x, y)
   */
  updateSurfaceExplicit(
    ...
    for (let j = 0; j <= segments; j++) {
      for (let i = 0; i <= segments; i++) {
        ...
        // Handle NaN and Infinity - clamp to reasonable range
        if (!Number.isFinite(z)) {
          z = 0; // Use 0 for invalid values
        } else {
          // Clamp to prevent extreme values
          z = Math.max(-100, Math.min(100, z));
        }

        // Coordinate mapping: math (x, y, z) -> Three.js (x, y, z) with Z up
        vertices.push(x, y, z);
      }
    }
    ...
    if (this.surfaceMesh) {
      ...
      this.surfaceMesh = new Mesh(geometry, this.material);
      this.scene.add(this.surfaceMesh);
    }
  }

  /**
   * Update the surface mesh for implicit function f(x, y, z) = 0
   * Uses Marching Cubes algorithm
   */
  updateSurfaceImplicit(
    ...
    const xStep = (xMax - xMin) / segments;
    const yStep = (yMax - yMin) / segments;
    const zStep = (zMax - zMin) / segments;
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      for (let j = 0; j <= segments; j++) {
        ...
        for (let k = 0; k <= segments; k++) {
          ...
          let value = evalFn(x, y, z);

          // Handle NaN and Infinity
          if (!Number.isFinite(value)) {
            value = 1000; // Treat as outside the surface
          }
        ...
        for (let k = 0; k < segments; k++) {
          ...
          for (let e = 0; e < 12; e++) {
            if (edgeTable[cubeIndex] & (1 << e)) {
              ...
              const t = val0 / (val0 - val1);

              edgeVertices[e] = [
                cubePositions[v0][0] +
                  t * (cubePositions[v1][0] - cubePositions[v0][0]),
                cubePositions[v0][1] +
                  t * (cubePositions[v1][1] - cubePositions[v0][1]),
                cubePositions[v0][2] +
                  t * (cubePositions[v1][2] - cubePositions[v0][2]),
              ];
            }
          }
